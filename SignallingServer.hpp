...
public:
    explicit SignallingServer(uint16_t port = DEFAULT_PORT)
        : m_port(port) {}
    ...
    bool Start() {
        ...
        std::cout << "[SignallingServer] Started on port " << m_port << std::endl;
        return true;
    }
    ...
    void Run() {
        ...
        while (m_running.load()) {
            ...
            if (clientSocket == INVALID_SOCKET) {
                ...
                if (err != WSAEINTR && err != WSAENOTSOCK && err != WSAESHUTDOWN) {
...
private:
    ...
    void processCommand(const std::shared_ptr<ClientContext>& client, const std::string& cmd) {
        if (cmd.compare(0, 9, "REGISTER ") == 0) {
            ...
            bool sameRegistration = false;
            {
                ...
                if (existing != m_idToClient.end() && existing->second.get() != client.get()) {
                    idTaken = true;
                } else {
                    sameRegistration = (client->isRegistered && client->clientId == id);

                    if (client->isRegistered && client->clientId != id) {
                        auto old = m_idToClient.find(client->clientId);
                        if (old != m_idToClient.end() && old->second.get() == client.get()) {
            ...
            std::cout << "[SignallingServer] Client "
                      << (sameRegistration ? "re-registered (same id): " : "registered: ")
                      << id << std::endl;
            return;
        }
        ...
        if (cmd.compare(0, 10, "CANDIDATE ") == 0) {
            ...
            if (target) {
                sendLine(target, "CANDIDATE " + client->clientId + " " + candidate + "\n");
            }
            return;
        }

        if (cmd.compare(0, 6, "OFFER ") == 0 || cmd.compare(0, 7, "ANSWER ") == 0) {
            ...
            auto target = findClientById(targetId);
            if (target) {
                sendLine(target, std::string(isOffer ? "OFFER " : "ANSWER ") + client->clientId + "\n");
            }
            return;
        }

        if (cmd.compare(0, 15, "OFFER_DECLINED ") == 0) {
            std::string targetId = cmd.substr(15);
            auto target = findClientById(targetId);
            if (target) {
                sendLine(target, "OFFER_DECLINED " + client->clientId + "\n");
            }
            return;
        }
        ...
        if (cmd.compare(0, 5, "CHAT ") == 0) {
            ...
            auto target = findClientById(targetId);
            if (target) {
                sendLine(target, "SERVER_MSG " + client->clientId + " " + message + "\n");
            }
            return;
        }
    ...
    bool sendLine(const std::shared_ptr<ClientContext>& client, const std::string& msg) {
