...
{
    CRITICAL_SECTION &cs;
    AutoCS(CRITICAL_SECTION &c) : cs(c)
    {
...
IceAgent::IceAgent(HANDLE completionPort)
    : m_completionPort(completionPort), m_maxRetries(3), m_retryTimeoutMs(2000),
...
{
    AutoCS lock(m_cs);
    return std::make_tuple(m_successNotified.load(), m_activeSocket.load(),
                           m_activeRemote);
}
...
void IceAgent::UpdateActivity()
{
    ...
    {
        if (pairPtr->state == PAIR_SUCCEEDED ||
...
void IceAgent::AddLocalCandidate(const Candidate &cand)
{
...

    {
        ...
        bool alreadyReceiving =
            (m_recvContexts.find(cand.sock) != m_recvContexts.end());
        m_localCandidates.emplace_back(cand);
        sock = cand.sock;
        needStartReceiving = (m_completionPort != NULL &&
                              sock != INVALID_SOCKET && !alreadyReceiving);
    }
...
void IceAgent::AddRemoteCandidate(const Candidate &cand)
{
...

    {
        ...
        std::cout << "[ICE] Added remote candidate: "
                  << (cand.type == HOST ? "host" : "srflx") << " " << cand.ip
                  << ":" << cand.port << std::endl;
...
bool IceAgent::AddNewPairsIncrementalInternal()
{
    ...
    std::vector<const LocalCandidate *> globalLocals;
    for (auto &l : m_localCandidates)
    {
        if (l.info.type == SERVER_REFLEXIVE)
            globalLocals.push_back(&l);
    }

    std::vector<const Candidate *> globalRemotes;
    for (auto &r : m_remoteCandidates)
    {
        if (r.type == SERVER_REFLEXIVE)
            globalRemotes.push_back(&r);
    }
    ...
    if (!globalLocals.empty() && !globalRemotes.empty())
    {
        for (const auto *local : globalLocals)
        {
            for (const auto *remote : globalRemotes)
            {
                std::string pairKey =
                    local->info.ip + ":" + std::to_string(local->info.port) +
                ...
                pair->priority = 0xFFFFFFFF00000000ULL;
                pair->retryCount = 0;
    ...
    if (added == 0 && globalLocals.empty() && globalRemotes.empty())
    {
        for (const auto &local : m_localCandidates)
        {
            for (const auto &remote : m_remoteCandidates)
            {
                if (local.info.type == HOST && remote.type == HOST)
                    continue;
                ...
                pair->state = PAIR_WAITING;
                uint32_t localPrio =
                    (local.info.type == SERVER_REFLEXIVE) ? 100 : 10;
                uint32_t remotePrio =
                    (remote.type == SERVER_REFLEXIVE) ? 100 : 10;
                pair->priority =
                    (static_cast<uint64_t>(localPrio) << 32) | remotePrio;
                pair->retryCount = 0;
                pair->lastActivity = std::chrono::steady_clock::now();
                m_pairs.push_back(pair);
                existingPairs.insert(pairKey);
    ...
    if (added > 0)
    {
        ...
        std::stable_sort(sorted.begin(), sorted.end(),
                         [](const std::shared_ptr<CandidatePair> &a,
                            const std::shared_ptr<CandidatePair> &b)
                         {
                             if (a->priority != b->priority)
                                 return a->priority > b->priority;
                             bool aGlobal =
                                 (a->local.type == SERVER_REFLEXIVE &&
                                  a->remote.type == SERVER_REFLEXIVE);
                             bool bGlobal =
                                 (b->local.type == SERVER_REFLEXIVE &&
                                  b->remote.type == SERVER_REFLEXIVE);
                             if (aGlobal != bGlobal)
                                 return aGlobal;
                             return false;
                         });
        ...
        std::cout << "[ICE] Added " << added
                  << " new pairs incrementally (total: " << m_pairs.size()
                  << ")\n";
        return true;
    }
...
bool IceAgent::StartConnectivityChecks(IceCallback callback)
{
...

    {
        ...
        {
            std::cerr << "[ICE] Cannot start checks: local="
                      << m_localCandidates.size()
        ...
        std::cout << "[ICE] Starting connectivity checks with "
                  << m_pairs.size() << " pairs" << std::endl;
    }
...
void IceAgent::StartNextBatch(size_t maxParallel)
{
    ...
    for (size_t i = currentIdx; i < m_pairs.size() && started < maxParallel;
         ++i)
    {
...
void IceAgent::StartReceiving(SOCKET sock, LocalCandidate *)
{
    ...
    {
        std::cerr << "[ICE] WSARecvFrom failed: " << WSAGetLastError()
                  << std::endl;
        return;
    }
...
void IceAgent::OnReceiveCompleted(IceUdpContext *ctx, DWORD bytesTransferred)
{
    ...
    agent->OnUdpPacket(ctx->socket, ctx->remoteAddr, ctx->buffer,
                       bytesTransferred);
...
void IceAgent::RearmReceiveContext(IceUdpContext *ctx)
{
    ...
    if (rc == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING)
    {
        std::cerr << "[ICE] WSARecvFrom re-arm failed: " << WSAGetLastError()
                  << std::endl;
    }
}
...
void IceAgent::FormPairsInternal()
{
    ...
    if (m_pairs.empty())
    {
        std::cout << "[ICE] No global pairs found, falling back to mixed "
                     "candidates\n";
    ...
    std::sort(m_pairs.begin(), m_pairs.end(),
                 ...
                 const std::shared_ptr<CandidatePair> &b)
              {
                  if (a->priority != b->priority)
                      return a->priority > b->priority;
                  bool aGlobal = (a->local.type == SERVER_REFLEXIVE &&
                                  a->remote.type == SERVER_REFLEXIVE);
                  bool bGlobal = (b->local.type == SERVER_REFLEXIVE &&
                                  b->remote.type == SERVER_REFLEXIVE);
                  if (aGlobal != bGlobal)
                      return aGlobal;
                  return false;
    ...
    std::cout << "[ICE] Formed " << m_pairs.size() << " pairs"
              << (globalLocals.empty() || globalRemotes.empty()
                      ? " (MIXED MODE)"
                      : " (GLOBAL ONLY MODE)")
              << std::endl;
}
...
bool IceAgent::SendBindingRequest(std::shared_ptr<CandidatePair> pair)
{
    ...
    std::cout << "[ICE] Sending STUN Binding Request to " << pair->remote.ip
              << ":" << pair->remote.port << " from socket " << pair->socket
              << "\n";

    int sent =
        sendto(pair->socket, reinterpret_cast<const char *>(&header),
...
void IceAgent::CheckPairTimeouts()
{
    ...
    for (auto &pairPtr : m_pairs)
    {
        if (pairPtr->state != PAIR_IN_PROGRESS)
            continue;

        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                           now - pairPtr->lastSent)
                           .count();
        ...
        if (elapsed > m_retryTimeoutMs)
        {
            if (pairPtr->retryCount < m_maxRetries)
            {
                ...
                std::cout << "[ICE] Retrying pair (attempt "
                          << pairPtr->retryCount << "/" << m_maxRetries
                          << "): " << pairPtr->local.ip << ":"
                          << pairPtr->local.port << "\n";

                if (pairPtr->activeTid.has_value())
                {
                    PendingKey oldKey{pairPtr->socket,
                                      pairPtr->activeTid.value()};
                    m_pendingRequests.erase(oldKey);
            ...
            {
                pairPtr->state = PAIR_FAILED;
                std::cout << "[ICE] Pair failed after " << m_maxRetries
                          << " attempts: " << pairPtr->local.ip << ":"
                          << pairPtr->local.port << "\n";
                needMorePairs = true;
            }
        }
...
void IceAgent::OnUdpPacket(SOCKET socket, const sockaddr_in &from,
                                          ...
                                          SOCKET recvSocket)
{
    ...
    if (it != m_pendingRequests.end())
    {
        ...
        std::cout << "[ICE] Matched pending request for pair: "
                  ...
                  << " -> " << matchedPair->remote.ip << ":"
                  << matchedPair->remote.port << "\n";
    ...
    const uint8_t *ptr =
        reinterpret_cast<const uint8_t *>(data + sizeof(StunHeader));
    ...
    while (ptr + 4 <= end)
    {
            ...
            value[1] == 0x01)
        {
            ...
            uint16_t realPort =
                ntohs(xorPort) ^ static_cast<uint16_t>(MAGIC >> 16);
    ...
    if (foundAddress && !m_successNotified.load())
    {
        ...
        {
            if (pPtr->state == PAIR_IN_PROGRESS)
                pPtr->state = PAIR_SUCCEEDED;
        }
...
void IceAgent::SendStunResponse(SOCKET sock, const sockaddr_in &from,
                                const uint8_t *tid)
{
    constexpr uint32_t MAGIC = 0x2112A442;
    ...
    uint8_t *attrPtr =
        reinterpret_cast<uint8_t *>(response + sizeof(StunHeader));
    ...
    *reinterpret_cast<uint16_t *>(attrPtr + 2) = htons(8);
    ...
    sendto(sock, response, sizeof(response), 0, (sockaddr *)&from,
           sizeof(from));
}
