...
public:
    ...
    static bool GetReflexiveAddress(const char* stunHost, uint16_t stunPort,
                                     SOCKET udpSocket, sockaddr_in& reflexiveAddr) {
        ...
        std::vector<uint8_t> request = BuildBindingRequest(transactionId.data());

        
        int sent = sendto(udpSocket, reinterpret_cast<const char*>(request.data()),
                          static_cast<int>(request.size()), 0,
                          (sockaddr*)&serverAddr, sizeof(serverAddr));
        if (sent == SOCKET_ERROR) {
            std::cerr << "[STUN] sendto failed: " << WSAGetLastError() << std::endl;
            return false;
        }
        ...
        if (recvLen == SOCKET_ERROR) {
            std::cerr << "[STUN] recvfrom failed: " << WSAGetLastError() << std::endl;
            return false;
        }

        
        return ParseBindingResponse(buffer, recvLen, transactionId.data(), reflexiveAddr);
    }
...
private:
    ...
    static std::vector<uint8_t> BuildBindingRequest(const uint8_t* transactionId) {
        StunHeader header{};
    ...
    static bool ParseBindingResponse(const char* buffer, int len,
        ...
        if (ntohs(header->type) != BINDING_RESPONSE) return false;
        if (ntohl(header->magicCookie) != 0x2112A442) return false;
        if (memcmp(header->transactionId, expectedTid, 12) != 0) return false;

        uint16_t attrLen = ntohs(header->length);
        const uint8_t* ptr = reinterpret_cast<const uint8_t*>(buffer + sizeof(StunHeader));
        const uint8_t* end = ptr + attrLen;

        while (ptr + 4 <= end) {
            const StunAttribute* attr = reinterpret_cast<const StunAttribute*>(ptr);
            ...
            if (type == XOR_MAPPED_ADDRESS && length >= 8) {
                ...
                uint16_t realPort = xorPort ^ htons(static_cast<uint16_t>(MAGIC >> 16));
