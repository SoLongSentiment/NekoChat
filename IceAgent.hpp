...
struct TidKey
{
    ...
    bool operator==(const TidKey &other) const
    {
...
struct CandidatePair
{
    ...
    CandidatePair()
        : socket(INVALID_SOCKET), state(PAIR_WAITING), priority(0),
          retryCount(0)
    {
    }
};
...
class IceAgent
{
  public:
    using IceCallback =
        std::function<void(bool success, const CandidatePair &pair)>;
    ...
    void OnUdpPacket(SOCKET socket, const sockaddr_in &from, const char *data,
                     int len);
    void Process();
    void Cancel();
    ...
    struct IceUdpContext
    {
        ...
        IceUdpContext(IceAgent *a, SOCKET s, LocalCandidate *lc = nullptr)
            : signature(SIGNATURE), agent(a), socket(s), flags(0),
              remoteAddrLen(sizeof(remoteAddr)), localCandidate(lc),
              detached(false)
        {
    ...
    void SendStunResponse(SOCKET sock, const sockaddr_in &from,
                          const uint8_t *tid);
    void HandleStunResponseInternal(const char *data, int len,
  ...
  private:
    ...
    std::unordered_map<PendingKey, std::weak_ptr<CandidatePair>, PendingKeyHash>
        m_pendingRequests;
